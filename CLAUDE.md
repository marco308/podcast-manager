...
- `spotify_playlist_id`: empty means one is created on the first run. Linking an existing playlist goes through `_check_spotify_playlist_link` in `routers/playlists.py`: the user must own it (`GET /playlists/{id}` owner vs `User.spotify_id`) and it can't already be linked to another managed playlist, because every rebuild fully replaces its contents. The form picks from `GET /api/playlists/spotify-playlists` (owned playlists only) rather than taking a free-text ID (issue #245).
