...
**Library sync** (`services/library_sync.py::sync_library`, shared by `POST /podcasts/sync` and the daily job): walks every page of `GET /me/shows`, upserts each show, then stamps `unfollowed_at` on every local podcast the walk didn't see. Three invariants:
...
- An **empty** `/me/shows` response against a non-empty local library is treated as a glitch, not a mass unfollow: nothing is stamped and `LibrarySyncResult.reconciled` is False (surfaced as `unfollow_check_skipped` on the endpoint, which the UI reports as a warning). Stamping there would take every playlist to nothing on the next build, and an empty response can't be told apart from a broken read.
