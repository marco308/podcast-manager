...
### Testing Against a Real Spotify Account Without Re-Login
...
- `daily_playlist_update` runs at 1 to `MAX_PLAYLIST_UPDATE_TIMES` (3) times a day, still as **one job**: several times become an `OrTrigger` of `CronTrigger`s, so APScheduler's default `max_instances=1` skips a run that would overlap the previous one. The job has `misfire_grace_time=600` and `coalesce=True` (APScheduler's default 1s grace dropped the run after a brief event-loop stall); job options survive `reschedule_job`, which only swaps the trigger. The times are mutable at runtime via `PUT /api/jobs/schedule` (`times: [{hour, minute}]`) and persisted in `app_settings` under `playlist_update_times` (`"HH:MM,HH:MM"`) so restarts pick them up. For installed iOS builds that only know one time, the endpoint still accepts `hour` + `minute`, `get_job_status` still sends `schedule` (the first time) next to `schedule_times`, and the legacy `playlist_update_hour` / `playlist_update_minute` keys are still written with the first time; `_load_update_times` falls back to them when the list key is absent.
- The run times are wall-clock times in the `TIMEZONE` setting (IANA name, default `UTC`, validated with `zoneinfo` at startup), passed to the `AsyncIOScheduler` and every `CronTrigger`, so they follow daylight saving there. `get_job_status` reports it as `schedule_timezone` and the web UI labels the times with it rather than presenting them as the browser's local time. `tzdata` is in `requirements.txt` because the slim image need not ship system zone data.
...
- `position` — per-playlist ordering, used when the playlist's `arrangement` is `by_position`. `PUT /playlists/{id}/podcasts/reorder` must list every assigned podcast exactly once (400 otherwise), since a partial or repeated list would leave two rows on one position
...
- `spotify_playlist_id`: empty means one is created on the first run. Linking an existing playlist goes through `_check_spotify_playlist_link` in `routers/playlists.py`: the user must own it (`GET /playlists/{id}` owner vs `User.spotify_id`) and it can't already be linked to another managed playlist, because every rebuild fully replaces its contents. The form picks from `GET /api/playlists/spotify-playlists` (owned playlists only) rather than taking a free-text ID (issue #245). `PATCH` follows the override convention — present-and-null unlinks (the Spotify playlist is left in place), absent leaves the link alone. The pre-check is racy on its own, so `uq_playlists_user_spotify_playlist` (migration 016) is the real guarantee and `_commit_playlist` turns its `IntegrityError` into a 409. A `PATCH` that changes both link and name flushes the link first (`_flush_playlist_link`), so a lost race is a 409 before the Spotify rename, not after. The picker stops at 20 pages (1000 playlists) and says so with `truncated: true`. Listing and checking private playlists needs the `playlist-read-private` scope, added in #245 — tokens issued before that need a fresh login.

**Fetching is proportional to the rule** (`PlaylistBuilder._fetch_unplayed`): a `newest` rule walks pages from offset 0 and stops once it has enough unplayed; an `oldest` rule reads `total` from the first page and walks backwards from the tail (if a tail page comes back empty, `total` overstated the catalogue — e.g. episodes not available in the market — and it falls back to a forward walk; nothing at all raises `EpisodeFetchError`, so the show is reported as failed rather than silently contributing nothing); an unlimited rule reads up to `MAX_EPISODES_PER_SHOW`. `podcast.unplayed_episodes` is only written when the walk saw the whole catalogue, together with `unplayed_counted_at`; limited rules never refresh it, so both UIs show the count's age and grey it out after 7 days, and there is deliberately no sorter on it (issue #241) — a dedicated counting job was rejected as API budget spent on a number no build reads. It is `NULL` ("not counted") until then; `POST /podcasts/sync` never writes it and makes no per-show episode calls (issue #155).
...
**Playlist ↔ Spotify:** `DELETE /playlists/{id}?remove_from_spotify=true` unfollows (= deletes) the Spotify playlist before removing the row; without it the Spotify playlist is left orphaned. Either way the delete holds `playlist_write_lock` (409 if a rebuild keeps it), so a rebuild can't be creating the Spotify playlist for a row being deleted. A rename via `PATCH` is pushed to Spotify first (`PUT /playlists/{id}`); a Spotify failure saves nothing (502), a Spotify 404 renames locally only.

**Single-user by construction:** `auth.py` closes registration once one `User` row exists, so the deployment has exactly one user. The lookup, count and insert run under `_registration_lock` (the app is one uvicorn process), so two simultaneous first sign-ins can't both register. `OWNER_SPOTIFY_ID`, when set, refuses every other Spotify account at the callback, first sign-in included; unset, the first account to sign in wins. Consequently `podcasts` is a **deliberately global table** — it has no `user_id`, and the podcast routes do not filter by owner. `playlists` *is* user-scoped (it predates the decision and the column is harmless), but nothing depends on that scoping for security. If multi-user is ever wanted, adding `Podcast.user_id` and filtering every podcast route is a prerequisite, not an optimisation (issue #154).

**Account deletion** (`DELETE /api/auth/me`, `services/account.py`, issue #266) is required in-app by App Store Guideline 5.1.1(v); both UIs offer it in Settings. It deletes every table explicitly, because SQLite here runs without `PRAGMA foreign_keys` and the declared `ondelete="CASCADE"` rules never fire. The global `podcasts` and `sync_logs` go only when no other user remains, and `app_settings` (server config) survives. Nothing may write for the user mid-delete: it first closes `locks.account_write_gate`, which every POST/PUT/PATCH/DELETE holds for its whole run (`AccountWriteGateMiddleware`, pure ASGI so `get_db`'s after-response commit stays inside), then takes `library_sync_lock` and `playlist_write_lock` for the jobs; 409 if any stays busy. Gate first, because a manual run holds the gate while waiting for the playlist lock. Without the gate, a write that validated its session before the delete leaves an orphan row, and SQLite reuses the freed user id, so the next account would inherit it. Spotify is untouched: its playlists stay, and Spotify has no token-revoke endpoint. With no user left, registration reopens.
...
**Sessions store only a hash:** `sessions.session_id` holds the hex SHA-256 of the cookie value (ORM attribute `Session.session_id_hash`; migration 020 hashed existing rows in place). `SessionService.get_session` hashes the cookie before looking it up, `create_session` returns the plaintext ID alongside the row because it is the only place it exists, and `delete_session` takes the row. `csrf_token` is stored as is. A loaded `Session` has no plaintext ID to hand back.

**Production hardening defaults:** the API docs and `openapi.json` exist only with `DEBUG`; SQL echo is its own `SQL_ECHO` setting and the engine always runs with `hide_parameters`; `SecurityHeadersMiddleware` (pure ASGI, like the write gate) adds `nosniff`, `X-Frame-Options: DENY`, `Referrer-Policy: no-referrer` and a `default-src 'none'` CSP to every response; `entrypoint.sh` trusts `X-Forwarded-For` only from private ranges (`FORWARDED_ALLOW_IPS` overrides).
