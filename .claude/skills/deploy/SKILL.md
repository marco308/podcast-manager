...
With `REGISTRY` set it deploys the images CI already published to GHCR, tagged with the checked-out commit — so `git pull` the commit you mean to ship first; `IMAGE_TAG=<sha>` rolls back to an earlier build, and `IMAGE_TAG=X.Y.Z` deploys a release (see the `release` skill). Without `REGISTRY` it builds `:latest` locally (single-node only). CI publishes `:latest` and `:<sha>` on every push to `main`, and also after each Dependabot auto-merge (the merge job dispatches CI on `main`).
